...
img = cv2.imread(inputName) # imagen en color
image = img[::2,::2,:]    # reducir a la mitad para acelerar proceso

# dimensiones imagen (ancho y alto)
...
# convert to image: equivalente a esta instrucción pero con bucles for para cada pixel:
# imageSeg = dataVector[:,:3].reshape(image.shape)
imageSeg = np.zeros(image.shape)
for y in range(imageH):
	for x in range(imageW):
	 	imageSeg[y,x,:] = [     int(dataVector[y * imageW + x][0]), 
...
if 1:
    ...
    cv2.waitKey(2)
    cv2.imshow("Segmentation",np.uint8(imageSeg))
    cv2.waitKey(2)   #cv2.waitKey(0)  # con esta opción sería necesario pulsar una tecla dentro de la imagen para salir
